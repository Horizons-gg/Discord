...
module.exports = function (app, main_client, mongodb) {
    ...
    client.login(process.env.BOT_ARES);
    ...
    function refresh() {
        fetch('http://10.0.0.2:7010')
            .then(res => res.json())
            .then(json => data = json)
            .catch(err => error = err)

        if (data !== undefined) {
            fs.writeFileSync('database/network/cache/10.0.0.2.json', JSON.stringify(data, null, '\t'))
        } else {
            fs.writeFileSync('database/network/cache/10.0.0.2.json', JSON.stringify({}, null, '\t'))
        }

        fs.readFile('database/network/10.0.0.2.json', 'utf8', (err, data) => {
